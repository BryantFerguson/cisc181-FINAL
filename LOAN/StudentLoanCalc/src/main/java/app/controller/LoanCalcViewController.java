...
public class LoanCalcViewController implements Initializable {
	...
	private void btnSetDefaultAdjustments(KeyEvent event) {
		this.txtAdjustPeriod.setText(NbrOfYears.getText());
		this.txtAdjustLength.setText(null);
		this.txtAdjustRateMax.setText(null);
	}
	...
	private boolean ValidateData() {
        ...
        if (LoanAmount.getText().trim().isEmpty() || !(Double.parseDouble(LoanAmount.getText().trim()) > 0)) {
            message.append("Loan Amount must be a positive double. \n");
            goodtogo = false;
        }
        		
        if(InterestRate.getText().trim().isEmpty() || Double.parseDouble(InterestRate.getText().trim()) < 0 
            || Double.parseDouble(InterestRate.getText().trim()) > 20 || InterestRate.getText().matches("[a-z]/i")) {
            message.append("Interest Rate must be a postive double between 0 and 20. \n");
            goodtogo = false;
        }

        
        if(NbrOfYears.getText().trim().isEmpty() || Integer.parseInt(NbrOfYears.getText().trim()) <= 0) {
            message.append("Number of years must be a positive value. \n");
            goodtogo = false;
        }
        
        
        if(EscrowAmount.getText().trim().isEmpty() || Double.parseDouble(EscrowAmount.getText().trim()) < 0){
            message.append("Escrow amount must be a positve double which is more than or equal to 0. \n");
            goodtogo = false;
        }
        
        
        if(AdditionalPayment.getText().trim().isEmpty() || Double.parseDouble(AdditionalPayment.getText().trim()) < 0) {
            message.append("Additional payment must be a postive double which is more than or equal to 0. \n");
            goodtogo = false;
        
        if (!goodtogo) {
            Alert fail = new Alert(AlertType.ERROR);
            fail.setHeaderText("Error!");
            fail.setContentText(message.toString());
            fail.showAndWait();
            return false;
            }
        }
	...
	private void btnCalcLoan(ActionEvent event) {
		...
		Loan loanExtra = new Loan(dLoanAmount, dInterestRate, dNbrOfYears, localDate, dAdditionalPayment, dEscrow,0,0,0);
		Loan loanNoExtra = new Loan(dLoanAmount, dInterestRate, dNbrOfYears, localDate, 0, dEscrow,0,0,0);
